...
bool lce_processJoinAccept (u8_t* jacc, u8_t jacclen, u16_t devnonce) {
    ...
    jacclen -= 4;
    u32_t mic1 = os_rmsbf4(jacc+jacclen);
#if defined(CFG_lorawan11)
    u8_t optneg = jacc[OFF_JA_DLSET] & JA_DLS_OPTNEG;
    if( optneg ) {
...
#endif
    os_getNwkKey(AESkey);
    u32_t mic2 = os_aes(AES_MIC|AES_MICNOAUX, jacc, jacclen);
#if defined(CFG_lorawan11)
    ...
    u8_t* nwkskey = LMIC.lceCtx.nwkSKey;
...
static void micB0 (u32_t devaddr, u32_t seqno, u8_t cat, int len) {
...
void lce_addMic (s8_t keyid, u32_t devaddr, u32_t seqno, u8_t* pdu, int len) {
    ...
    const u8_t* key = LMIC.lceCtx.nwkSKey;
    os_copyMem(AESkey,key,16);
