...
static const u16_t LORA_RXDONE_FIXUP_125[] = {
    [FSK]  =     us2osticks(0),
    [SF7]  =     us2osticks(0),
    [SF8]  =  us2osticks(1648),
    [SF9]  =  us2osticks(3265),
...
static const u16_t LORA_RXDONE_FIXUP_500[] = {
    [FSK]  = us2osticks(    0),
    [SF7]  = us2osticks(    0),
    [SF8]  = us2osticks(    0),
    [SF9]  = us2osticks(    0),
...
static u8_t readReg (u8_t addr) {
    hal_spi_select(1);
    hal_spi(addr & 0x7F);
    u8_t val = hal_spi(0x00);
    hal_spi_select(0);
    return val;
}

// (used by perso)
void radio_writeBuf (u8_t addr, u8_t* buf, u8_t len) {
    hal_spi_select(1);
    hal_spi(addr | 0x80);
...
// (used by  perso)
void radio_readBuf (u8_t addr, u8_t* buf, u8_t len) {
    hal_spi_select(1);
    hal_spi(addr & 0x7F);
    for (u8_t i = 0; i < len; i++) {
        buf[i] = hal_spi(0x00);
...
static void setopmode (u8_t opmode) {
...
static void configChannel (void) {
    ...
    u32_t frf = ((u64_t)LMIC.freq << 19) / 32000000;
    writeReg(RegFrfMsb, frf >> 16);
    writeReg(RegFrfMid, frf >> 8);
    writeReg(RegFrfLsb, frf >> 0);
}

static void setRadioConsumption_ua (bool boost, u8_t pow) {
    u32_t ua;
#if defined(BRD_sx1276_radio)
    static const u16_t BOOSTPOW[19] = { /* 2-20 */
        35140  >> 1,
        36770  >> 1,
        38770  >> 1,
        40140  >> 1,
    ...
    static const u16_t RFOPOW[16] = { /* 0-15 */
        15910,
        16760,
        17570,
        18530,
...
static void txfsk (bool txcont) {
    ...
    if (!txcont) {
        ...
        // set tx timeout
        radio_set_irq_timeout(os_getTime() + us2osticks((u32_t)(FIFOTHRESH+10)*8*1000/50));
    }
...
static void rxfsk (bool rxcontinuous) {
    ...
    if (rxcontinuous) {
        ...
        // set rx timeout
        radio_set_irq_timeout(LMIC.rxtime + us2osticks((u32_t)(2*FIFOTHRESH)*8*1000/50));
        // busy wait until exact rx time
        ostime_t now = os_getTime();
...
bool radio_irq_process (ostime_t irqtime, u8_t diomask) {
    ...
    // dispatch modem
    if (isFsk(LMIC.rps)) { // FSK modem
        u8_t irqflags1 = readReg(FSKRegIrqFlags1);
        u8_t irqflags2 = readReg(FSKRegIrqFlags2);
        ...
        if (irqflags2 & IRQ_FSK2_PACKETSENT_MASK) { // TXDONE
            ...
            // update tx timeout
            radio_set_irq_timeout(irqtime + us2osticks((u32_t)(FIFOTHRESH+10)*8*1000/50));
        ...
        u8_t irqflags = readReg(LORARegIrqFlags);
        ...
        if (irqflags & IRQ_LORA_TXDONE_MASK) { // TXDONE
            ...
            debug_printf("RX[rssi=%d,snr=%.2F,len=%d]: %.80h\r\n",
                         LMIC.rssi - RSSI_OFF, (s32_t)(LMIC.snr * 100 / SNR_SCALEUP), 2,
