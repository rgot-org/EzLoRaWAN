...
static void SetModulationParamsLora (u16_t rps) {
    uint8_t param[4];
...
static void SetPacketParamsLora (u16_t rps, int len, int inv) {
    uint8_t param[6];
...
static void SetPacketParamsFsk (u16_t rps, int len) {
    uint8_t param[9];
...
static void GetPacketStatusLora (s8_t *rssi, s8_t *snr) {
    uint8_t buf[3];
...
static s8_t GetPacketStatusFsk (void) {
    uint8_t buf[3];
    readcmd(CMD_GETPACKETSTATUS, buf, 3);
