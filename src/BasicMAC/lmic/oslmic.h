...
#define osticks2sec(os)  ((s32_t)(((os)               ) / OSTICKS_PER_SEC))
#define osticks2ms(os)   ((s32_t)(((os)*(s64_t)1000    ) / OSTICKS_PER_SEC))
#define osticks2us(os)   ((s32_t)(((os)*(s64_t)1000000 ) / OSTICKS_PER_SEC))
// Special versions
...
#define osticks2secCeil(os) ((s32_t)(((os) + (OSTICKS_PER_SEC - 1)) / OSTICKS_PER_SEC))
// Extended versions
...
#ifndef os_getNwkKey
void os_getNwkKey (u8_t* buf);
#endif
#ifndef os_getAppKey
void os_getAppKey (u8_t* buf);
#endif
#ifndef os_getJoinEui
void os_getJoinEui (u8_t* buf);
#endif
#ifndef os_getDevEui
void os_getDevEui (u8_t* buf);
#endif
#ifndef os_getRegion
u8_t os_getRegion (void);
#endif
...
void os_wlsbf4 (u8_t* buf, u32_t value);
#endif
#ifndef os_rmsbf4
...
void os_wmsbf4 (u8_t* buf, u32_t value);
#endif
#ifndef os_rlsbf2
...
void os_wlsbf2 (u8_t* buf, u16_t value);
#endif
#ifndef os_rmsbf2
...
void os_wmsbf2 (u8_t* buf, u16_t value);
#endif
#ifndef os_wlsbf3
//! Write 24-bit quantity into buffer in little endian byte order.
void os_wlsbf3 (u8_t* buf, u32_t value);
#endif

//! Get random number (default impl for u16_t).
#ifndef os_getRndU2
#define os_getRndU2() ((u16_t)((os_getRndU1()<<8)|os_getRndU1()))
#endif
#ifndef os_crc16
u16_t os_crc16 (u8_t* d, uint len);
#endif

#if defined(CFG_budha) // XXX:obsoleted by budha2 (service)
// HAL support required by BUDHA:
u8_t* os_getWSleepParams(void);
void  os_setWSleepParams(u8_t* params);
int   os_checkSignature(const u8_t* buf, u8_t len);
u32_t  os_getFWCRC(void);
int   os_usedWakeNonce(u16_t nonce);
void  os_commitWakeNonce(u16_t nonce);
enum { BOOT_WSLEEP=0, BOOT_OS };
void  os_setBootMode(u8_t mode);
void  os_boot(void);
s16_t  os_fwChunk(u8_t* p, u8_t len);
#endif // defined(CFG_budha)

#endif // !HAS_os_calls

// public radio functions
void radio_irq_handler (u8_t dio, ostime_t ticks); // (used by EXTI_IRQHandler)
void radio_init (bool calibrate); // (used by os_init())
void radio_writeBuf (u8_t addr, u8_t* buf, u8_t len); // (used by perso)
void radio_readBuf (u8_t addr, u8_t* buf, u8_t len); // (used by perso)
void radio_set_irq_timeout (ostime_t timeout);

// radio-specific functions
bool radio_irq_process (ostime_t irqtime, u8_t diomask);
...
void radio_cad (void);
void radio_cw (void);
void radio_generate_random (u32_t *words, u8_t len);
