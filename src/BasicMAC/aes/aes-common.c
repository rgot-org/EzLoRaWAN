...
static void os_aes_cmac(uint8_t *buf, u16_t len, uint8_t prepend_aux) {
    if (prepend_aux)
...
static void os_aes_ctr (uint8_t *buf, u16_t len) {
    uint8_t ctr[16];
    while (len) {
...
u32_t os_aes (uint8_t mode, uint8_t *buf, u16_t len) {
