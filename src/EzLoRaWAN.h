...
public:
	...
	/// Disallow copying
	///
	EzLoRaWAN(const EzLoRaWAN& ref) = delete;
	///
	/// set lmic_pins
	///
	void setPins(uint8_t nss, uint8_t rx, uint8_t tx, uint8_t rst, uint8_t dio0, uint8_t dio1, uint8_t dio2, uint8_t busy, uint8_t tcxo);
	...
	/// @param rx The pin for rx control
	/// @param tx The pin for tx control
	/// @param rst The pin for reset
	/// @param dio0 The DIO0 pin
	/// @param dio1 The DOI1 pin
	/// @param dio2 The DIO2 pin
	/// @param busy The Busy pin
	/// @param tcxo The tcxo pin
	/// @return True if the radio has been initialised, false if not
	///
	bool begin(uint8_t nss, uint8_t rx, uint8_t tx, uint8_t rst, uint8_t dio0, uint8_t dio1, uint8_t dio2, uint8_t busy, uint8_t tcxo);
	...
	/// @return True if the radio has been initialised, false if not
	///
	bool begin(const lmic_pinmap* pPinmap);
...
private:
	...
	/// This is a friend so the function can access the private variables of this class so it can be declared globally
	///
	friend void onLmicEvent(ev_t ev);
	friend void os_getJoinEui(xref2u1_t buf);
	friend void os_getNwkKey(xref2u1_t buf);
	friend uint8_t os_getRegion(void); 
